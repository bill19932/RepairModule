...
// Enhance image for better OCR of handwritten text
const enhanceImageForHandwriting = async (
  dataUrl: string,
): Promise<string> => {
  return new Promise<string>((resolve, reject) => {
    ...
    img.onload = () => {
      try {
        ...
        // Enhance contrast and brightness for handwritten text
        // This helps Tesseract recognize handwriting better
        for (let i = 0; i < data.length; i += 4) {
          ...
          // Increase contrast using S-curve for better text definition
          let enhanced = gray;

          // Apply contrast stretch to increase difference between dark (text) and light (background)
          if (gray < 128) {
            // Darken darker pixels
            enhanced = Math.max(0, gray - 20);
          } else {
            // Brighten lighter pixels
            enhanced = Math.min(255, gray + 20);
          }

          // Apply some additional sharpening by comparing to neighbors
          data[i] = enhanced;
          data[i + 1] = enhanced;
          data[i + 2] = enhanced;
          data[i + 3] = a;
        ...
        // Additional sharpening pass for text clarity
        const sharpened = ctx.getImageData(0, 0, canvas.width, canvas.height);
        const sharpData = sharpened.data;

        // Apply light sharpening kernel
        const kernel = [0, -0.25, 0, -0.25, 2, -0.25, 0, -0.25, 0];
        const kernelSize = 3;
        const kernelOffset = Math.floor(kernelSize / 2);
        const tempData = new Uint8ClampedArray(sharpData);

        for (let y = kernelOffset; y < canvas.height - kernelOffset; y++) {
          for (let x = kernelOffset; x < canvas.width - kernelOffset; x++) {
            let value = 0;
            for (let ky = 0; ky < kernelSize; ky++) {
              for (let kx = 0; kx < kernelSize; kx++) {
                const idx =
                  ((y - kernelOffset + ky) * canvas.width +
                    (x - kernelOffset + kx)) *
                  4;
                value += tempData[idx] * kernel[ky * kernelSize + kx];
              }
            }
            const pixelIdx = (y * canvas.width + x) * 4;
            const result = Math.max(0, Math.min(255, value));
            sharpData[pixelIdx] = result;
            sharpData[pixelIdx + 1] = result;
            sharpData[pixelIdx + 2] = result;
          }
        }
